...
export class RenderTrigger extends HTMLElement {
  constructor() {
    ...
    triggerTemplate.innerHTML =
      '<button id="trigger">Trigger rendering</button><button id="select15min">Select 15 mins</button>';
    this._shadowRoot.appendChild(triggerTemplate.content.cloneNode(true));
    this._shadowRoot
      .querySelector('#trigger')
      .addEventListener('click', this.triggerRendering);
    this._shadowRoot
      .querySelector('#select15min')
      .addEventListener('click', this.triggerSelectChange);
  }
